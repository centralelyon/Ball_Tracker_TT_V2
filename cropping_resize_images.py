...
for i in glob.glob('image\*.jpg'):
    ...
    elmt2 = 'RGBI'
    name_small = ''.join([i for i in name if i not in elmt2])
    img=cv2.imread(i)
    data_csv.seek(0)
    for line in reader:
        if line[1] == name_small:
    ...
    for j in noms:
        ...
        if os.path.isfile(os.path.join('Cropped',j,'imager', name+'.jpg'))==False:
            cv2.imwrite(os.path.join('Cropped',j,'imager', name+'.jpg'),cropped_image)
